...
function(exports, scene, scenegraph, node) {
	...
	function move(node, parameter) {
		...
		if (parameter) {
			...
			var moveSpeed = parameter.movementSpeed;
		}
		if (moveSpeed === undefined) {
			moveSpeed = [Math.random(), Math.random(), Math.random()];
		}
		...
		return function () {
			let currentPos = node.transformation.translate;
			let newPos = node.transformation.translate;
			newPos[0] += moveSpeed[0];
			newPos[1] += moveSpeed[1];
			newPos[2] += moveSpeed[2];

			if (newPos[0] - boundingBoxMin[0] <= sphereRadius || boundingBoxMax[0] - newPos[0] <= sphereRadius) {
				moveSpeed[0] = -1.0 * moveSpeed[0];
			}
			if (newPos[1] - boundingBoxMin[1] <= sphereRadius || boundingBoxMax[1] - newPos[1] <= sphereRadius) {
				moveSpeed[1] = -1.0 * moveSpeed[1];
			}
			if (newPos[2] - boundingBoxMin[2] <= sphereRadius || boundingBoxMax[2] - newPos[2] <= sphereRadius) {
				moveSpeed[2] = -1.0 * moveSpeed[2];
			}
			node.translate(moveSpeed);
			return true;
		}
